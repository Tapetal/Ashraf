...
<!--
  IMPORTANT: position:relative on the outer div so DotGrid (absolute inset-0)
  fills it correctly. The inner content div needs relative + z-10 so it sits
  ABOVE the canvas layer.
-->
<div class="relative min-h-screen bg-[#f8fafc] dark:bg-[#070b0f]">
  <DotGrid />

  <!-- z-10 lifts content above the canvas -->
  <div class="relative z-10 pt-24 pb-24 px-4 sm:px-6">
          ...
          <button
            ...
            class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold border transition-all duration-200
              {activeFilter === val
                ...
                : 'text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 bg-white dark:bg-surface-800 hover:border-zinc-300 dark:hover:border-zinc-700 hover:text-zinc-700 dark:hover:text-zinc-300'}"
          >
            {label}
          </button>
        {/each}
        ...
        {#each filtered as project (project.id)}
          <article class="group rounded-2xl border overflow-hidden flex flex-col transition-all duration-300
            ...
            <div class="h-1 w-full flex-shrink-0"
              style="background: linear-gradient(90deg, {project.accent}, {project.accent}55)"></div>
            ...
            <div class="p-5 sm:p-6 flex flex-col flex-1">
              ...
              <h2 class="text-base sm:text-lg font-black text-zinc-900 dark:text-zinc-100 mb-2
                group-hover:text-zinc-700 dark:group-hover:text-white transition-colors">
                {project.title}
              </h2>
              ...
              <div class="flex flex-wrap gap-1.5 mb-5">
                {#each project.tags.slice(0, 4) as tag}
                  <span class="text-[10px] font-mono px-2 py-0.5 rounded-md
                    bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-500">
                    {tag}
                  </span>
                {/each}
              ...
              <div class="flex items-center gap-3 mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800">
                ...
                {#if project.repo}
                  <a href={project.repo} target="_blank" rel="noopener"
                    class="flex items-center gap-1.5 text-xs font-semibold ml-auto
                      text-zinc-400 dark:text-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors">
