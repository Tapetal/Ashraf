<script lang="ts">
  ...
  // Logical (design) size — we always draw at this resolution then scale
  const LW = 680, LH = 400;
  ...
  function updateHUD() {
    accuracy = attempts > 0 ? `${Math.round((goals / attempts) * 100)}%` : '';
  }
  ...
  // ── Rubber-band keeper AI ─────────────────────────────────────────────────
  function decideKeeperDive(shotZone: Zone) {
    let diveZone: Zone;
    const others = (['left','center','right'] as Zone[]).filter(z => z !== shotZone);

    if (attempts < 3) {
      // First 3 shots: keeper always correct (builds tension)
      diveZone = shotZone;

    } else if (attempts === 3) {
      // 4th shot: guaranteed miss — give player first goal
      diveZone = others[Math.floor(Math.random() * others.length)];

    } else {
      // Rubber-band dynamic difficulty
      const recentMisses = Math.max(0, attempts - goals);
      let keeperSkill = 0.60;
      if (recentMisses >= 3) keeperSkill = 0.25; // struggling → easier keeper
      if (streak >= 4)       keeperSkill = 0.82; // hot streak → harder keeper

      // Pattern bias (reads shot history)
      const recent    = shotHistory.slice(-5);
      const sameCount = recent.filter(z => z === shotZone).length;
      if (recent.length >= 3) keeperSkill += (sameCount / recent.length) * 0.15;
      keeperSkill = clamp(keeperSkill, 0.15, 0.90);
    ...
    const diveY = GOAL.cy + GOAL.h * (power > 0.72 ? 0.08 : 0.12 + Math.random() * 0.4);

    keeper.diveTargetX  = diveX;
    keeper.diveTargetY  = diveY;
    keeper.diveAngle    = diveZone === 'left' ? -0.7 : diveZone === 'right' ? 0.7 : 0;
    keeper.diveSpeed    = attempts < 3 ? 0.08 : clamp(0.042 + goals * 0.008, 0.042, 0.10);
    keeper.reactionTimer = attempts < 3 ? 10 : clamp(24 - goals * 2, 7, 24);
  ...
  function shoot() {
    if (phase !== 'aim') return;
    const tx = clamp(mouseX, GOAL.cx - GOAL.w/2 + 14, GOAL.cx + GOAL.w/2 - 14);
    const ty = clamp(
      mouseY < GOAL.cy + GOAL.h + 60 ? mouseY : GOAL.cy + GOAL.h * 0.4,
      GOAL.cy + 6, GOAL.cy + GOAL.h - 6
    );
  ...
  function resolveShot() {
    ...
    const shotZone = getZone(ball.tx);

    let saved = false;
    if (inGoal) {
      if (keeper.diveZone === shotZone) {
        saved = Math.random() < (power > 0.72 ? 0.30 : 0.78);
      } else {
        saved = Math.random() < 0.06;
      }
    }
    ...
    if (inGoal && !saved) {
      ...
      lastResult  = 'SAVED';
      sideMessage = 'Keeper guessed right. Try the opposite corner.';
    } else {
      streak = 0;
      bigText = { text:'MISSED', color:'#fbbf24', alpha:1.2 };
      lastResult  = 'MISSED';
      sideMessage = 'Aim inside the goal.';
    }
  ...
  function drawPitch() {
    ...
    for (let i=0;i<12;i++) {
      if (i%2===0) { ctx.fillStyle='rgba(255,255,255,0.018)'; ctx.fillRect((i/12)*LW,0,LW/12,LH); }
    }
  ...
  function drawNet() {
    if (!ctx) return;
    const gx=GOAL.cx-GOAL.w/2+GOAL.post, gy=GOAL.cy+GOAL.post;
    const gw=GOAL.w-GOAL.post*2, gh=GOAL.h;
    ...
    for (let c=0;c<=10;c++) {
      const nx=gx+(c/10)*gw+(ball.active&&ball.progress>0.88?wave*0.3:0);
      ctx.beginPath(); ctx.moveTo(nx,gy); ctx.lineTo(nx,gy+gh); ctx.stroke();
    }
    for (let r=0;r<=4;r++) {
      const ny=gy+(r/4)*gh;
      ctx.beginPath(); ctx.moveTo(gx,ny+(r>0?wave:0)); ctx.lineTo(gx+gw,ny+(r>0?wave:0)); ctx.stroke();
    }
  }
  ...
  function drawGoalPosts() {
    ...
    ctx.beginPath();
    ctx.moveTo(GOAL.cx-GOAL.w/2, GOAL.cy+GOAL.h+24);
    ctx.lineTo(GOAL.cx-GOAL.w/2, GOAL.cy);
    ctx.lineTo(GOAL.cx+GOAL.w/2, GOAL.cy);
    ctx.lineTo(GOAL.cx+GOAL.w/2, GOAL.cy+GOAL.h+24);
  ...
  function drawBall() {
    ...
    for (let i=0;i<5;i++) {
      ...
      ctx.beginPath(); ctx.arc(Math.cos(a)*r*0.44,Math.sin(a)*r*0.44,r*0.24,0,Math.PI*2);
      ctx.fillStyle='rgba(20,20,20,0.22)'; ctx.fill();
    }
    ctx.restore();
  }
  ...
  function loop() {
    ...
    if (shake.dur>0) {
      shake.t++; const prog=shake.t/shake.dur; const m=shake.mag*(1-prog);
      shake.x=(Math.random()-0.5)*m; shake.y=(Math.random()-0.5)*m;
      if (shake.t>=shake.dur) shake.dur=0;
  ...
  // ── Canvas setup — key fix: canvas stays at LW×LH logical pixels ─────────
  function setupCanvas() {
    if (!canvas || !wrapper) return;
    const dpr = window.devicePixelRatio || 1;
    // Always render at LW×LH, CSS scales it via width:100%
  ...
  // Translate real pointer → logical canvas coords
  ...
  onMount(() => {
    ...
    canvas.addEventListener('touchmove', (e) => {
      ...
      mouseX=x; mouseY=y; aimVisible=true;
    }, { passive: false });
    ...
    canvas.addEventListener('touchstart', (e) => {
      ...
      if (phase!=='aim') return;
      mouseHeld=true; holdStart=performance.now(); power=0;
    }, { passive: false });
    ...
    // Resize only re-applies the DPR transform — logical size never changes
  ...
  <div class="mx-auto max-w-6xl">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-8">
      ...
      <p class="max-w-sm text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
        Aim, charge your shot, and outguess the keeper. Keeper adapts to your patterns.
      </p>
    </div>
    ...
    <div class="flex flex-col gap-5 lg:grid lg:grid-cols-[1fr_260px] items-start">
      ...
      <!-- Canvas — aspect-ratio keeps height proportional on all screen sizes -->
      <div
        bind:this={wrapper}
      ...
      <div class="flex flex-col gap-3 w-full">
        ...
        <div class="rounded-[24px] border border-white/8 bg-white/5 p-4">
          <p class="text-[9px] font-mono uppercase tracking-[0.3em] text-zinc-500 mb-2">Controls</p>
          <ul class="space-y-1.5 text-xs text-zinc-500 leading-relaxed">
            <li><span class="text-zinc-800 dark:text-white font-medium">Aim</span> — move cursor / drag finger toward goal</li>
            <li><span class="text-zinc-800 dark:text-white font-medium">Power</span> — hold to charge bar</li>
            <li><span class="text-zinc-800 dark:text-white font-medium">Shoot</span> — release to fire</li>
            <li class="text-zinc-400 dark:text-zinc-600 pt-1">Goal splits into left · center · right. Keeper picks one zone.</li>
          </ul>
        </div>
